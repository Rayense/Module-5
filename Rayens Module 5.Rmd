---
title: "Emily Rayens Module 5"
subtitle: "https://github.com/Rayense/Module-5"
author: "Emily Rayens"
date: "May 19, 2020"
output:
...
ggplot(data=mers) + 
  ...
  labs(x='Epidemic Day', y='Case Count', title='Global count of MERS cases',
...
*I used GitKraken to push all changes to the Github repository*
...
## Summary of MERS Analysis from Module 1
*In conducting a brief summary of the earlier analysis, we need to begin by recoding to simplify the outcomes and calculating the infectious period and coverting it into days.* 
```{r}
mers$outcome2 <- recode(mers$outcome, "''='Unknown'; 'fatal'='Fatal'; 'recovered'='Recovered'; '?recovered'='Suspect_Recovered'; 'recovered?'='Suspect_Recovered'; '?fatal'='Suspect_Fatal'; 'fatal?'='Suspect_Fatal'")

mers$infectious.period <- mers$hospitalized2-mers$onset2 # calculate infectious period
...
*We can then plot some overview figures of the infectious period in the MERS data*
```{r}
ggplot(data=mers, mapping=aes(x=epi.day, y=infectious.period2)) + 
  geom_point(mapping = aes(color=country)) +
  facet_grid(outcome2 ~ country) + 
  scale_y_continuous(limits = c(0, 50)) +
   theme(axis.text.x = element_text(angle = 90))+
  scale_x_continuous(breaks=c(0, 1000, 2000))+
   labs(x='Epidemic day', y='Infectious period',
       title='MERS Infectious Period', caption="Data from: https://github.com/rambaut/MERS-Cases/blob/gh-pages/data/cases.csv")
```

```{r}
ggplot(data=mers, color=country) + 
  geom_violin(mapping = aes(x=epi.day, y=outcome2, color=country)) +
   labs(x='Epidemic day', y='Outcome',
       title='MERS Case Fatality Rate', caption="Data from: https://github.com/rambaut/MERS-Cases/blob/gh-pages/data/cases.csv")+
theme_solarized(light=FALSE) +
 scale_colour_solarized('blue')
```

*We can simplify the violin plot of the infectious disease period of the major players in the MERS outbreak* 
```{r}
ggplot(data=mers, mapping=aes(x=epi.day, y=infectious.period2)) + 
  geom_violin(mapping = aes(color=country)) +
  scale_y_continuous(limits = c(0, 50)) +
...
*This is an interactive plot of the MERS Case Fatality Rate*
```{r}
finalthing <-ggplot(data=mers, mapping=aes(x=epi.day, y=infectious.period2)) + 
  geom_violin(mapping = aes(color=country)) +
  scale_y_continuous(limits = c(0, 50)) +
   labs(x='Epidemic day', y='Infectious period', title='MERS infectious period (positive values only) over time', caption="Data from: https://github.com/rambaut/MERS-Cases/blob/gh-pages/data/cases.csv")
ggplotly(finalthing) 
...
*KSA was most affected by MERS over the long term, with a relatively stable infectious period and mortality over almost the entirety of the epidemic duration (almost 4000 days). South Korea and the UAE saw small outbreaks but these were controlled over the course of roughly 1-2 years. The mortality rates in each country are difficult to determine as each had a high percentage of unknown outcome. However, South Korea saw the highest rate of recovery within their acute outbreak period. Overall, MERS has a moderate fatality rate and high enough infectivity to persist in these populations.*
